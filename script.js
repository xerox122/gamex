...
function spinWheel() {
    ...
    // Shuffle and ensure no contiguous duplicates
    let displayPrizes = smartWatchWon ? allPrizes.filter(prize => prize !== "Smart Watch") : [...allPrizes];
    displayPrizes = shuffleArray(displayPrizes);
    displayPrizes = ensureNoContiguousDuplicates(displayPrizes);
    ...
    function rotate() {
        ...
        if (spinTime >= spinTimeTotal) {
            ...
            // Select a random prize from the shuffled displayPrizes array
            let winningIndex = Math.floor(Math.random() * displayPrizes.length);
            const winningOutcome = displayPrizes[winningIndex];
...
drawWheel(allPrizes);  // Initialize the wheel with the default prize list
